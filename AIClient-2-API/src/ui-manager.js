...
function getExpiryTime() {
    const now = Date.now();
    const expiry = 100 * 365 * 24 * 60 * 60 * 1000; // 100年（永不过期）
    return now + expiry;
}
...
export async function handleUIApiRequests(method, pathParam, req, res, currentConfig, providerPoolManager) {
    ...
    if (method === 'POST' && pathParam === '/api/config') {
        try {
            ...
            if (newConfig.USAGE_AUTO_REFRESH_INTERVAL !== undefined) currentConfig.USAGE_AUTO_REFRESH_INTERVAL = newConfig.USAGE_AUTO_REFRESH_INTERVAL;
            ...
            try {
                ...
                const configToSave = {
                    ...
                    USAGE_AUTO_REFRESH_INTERVAL: currentConfig.USAGE_AUTO_REFRESH_INTERVAL
                };
